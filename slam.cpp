...
void SensorThread(TThreadParams params) {
    ...
    try {
        ...
        if (!sensor) {
            throw runtime_error(string("***ERROR***: Class name not recognized: ") + driver_name);
        }
        ...
        const int process_period_ms = mrpt::round(1000.0 / sensor->getProcessRate());
        ...
        while (!allThreadsMustExit) {
            ...
            if (At_rem_ms > 0) this_thread::sleep_for(chrono::milliseconds(At_rem_ms));
        }
...
void MapBuilding_ICP_Live(const string &INI_FILENAME) {
    ...
    const string OUT_DIR_STD = iniFile.read_string("MappingApplication", "logOutput_dir", "log_out", true);
    const int LOG_FREQUENCY = iniFile.read_int("MappingApplication", "LOG_FREQUENCY", 5, true);
    const bool SAVE_IMAGE = iniFile.read_bool("MappingApplication", "SAVE_IMAGE", false, true);
        ...
        CObservation2DRangeScan::Ptr observation;
        {
            ...
            for (mrpt::hwdrivers::CGenericSensor::TListObservations::
                ...
                if (it->second && IS_CLASS(it->second, CObservation2DRangeScan)) {
                    observation = dynamic_pointer_cast<CObservation2DRangeScan>(it->second);
                }
            }
        }
